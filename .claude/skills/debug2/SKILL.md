...
Trigger: `/debug2 <description of the bug or where it was observed>`
...
- **Then: Find or write a test** that demonstrates the bug:
  - **Unit/integration test** (vitest): If the bug is in a library function, API route, or logic module, write a failing test.
  - **E2E test** (Playwright): If the bug is in the UI, involves timing/race conditions, or requires a running app, use Playwright to reproduce it in the browser.
...
2. **Add temporary logging** at decision points along this path and **run the reproduction** to see actual values:
   ...
   - Run the test or Playwright scenario and read the output.
...
- **Automated tests**: Always try to leave behind a regression test so the bug can't recur silently. If a regression test isn't practical (e.g. the bug requires simulating HMR or other environment conditions that can't be reproduced in a test runner), document why in the summary.
